...
# Transaction is an SkyHigh block chain transaction.
type Transaction {
...
# Delegation represents a delegation on SkyHigh block chain.
type Delegation {
    ...
    # outstandingSSKH represents the amount of sSKH tokens representing
    # the tokenized stake minted and un-repaid on this delegation.
    outstandingSSKH: BigInt!

    # tokenizerAllowedToWithdraw indicates if the stake tokenizer allows the stake
    # to be withdrawn. That means all the sSKH tokens have been repaid and the sSKH
...
# Address is a 20 byte SkyHigh address, represented as 0x prefixed hexadecimal number.
scalar Address
...
# SkhBlockBurn represents a native SKH tokens burn record per created block.
type SkhBlockBurn {
    ...
    # amount represents the amount of SKH tokens burned in WEI units (18 digits fixed INT encoded as HEX number).
    amount: BigInt!

    # skhValue represents SKH value of the burned SKH tokens.
    skhValue: Float!
}
...
# Price represents price information of core SkyHigh token
type Price {
    "Source unit symbol."
    fromSymbol: String!
...
type Query {
    ...
    # Total number of accounts active on the SkyHigh blockchain.
    accountsActive:Long!
    ...
    # Get the id of the current epoch of the SkyHigh blockchain.
    currentEpoch:Long!
    ...
    # The last staker id in SkyHigh blockchain.
    lastStakerId: Long!

    # The number of stakers in SkyHigh blockchain.
    stakersNum: Long!
    ...
    # Get price details of the SkyHigh blockchain token for the given target symbols.
    price(to:String!):Price!

    # Get calculated staking rewards for an account or given
    # staking amount in SKH tokens.
    ...
    # by the Uniswap Core contract on SkyHigh blockchain.
    ...
    # skhBurnedTotal provides the total amount of native SKH tokens burned
    # by the chain from paid transaction fees in WEI units.
    skhBurnedTotal: BigInt!

    # skhBurnedTotalAmount provides the total amount of native SKH tokens burned
    # by the chain from paid transaction fees in SKH units.
    skhBurnedTotalAmount: Float!

    # skhLatestBlockBurnList provides a list of latest burned native SKH tokens per-block.
    skhLatestBlockBurnList(count: Int = 25): [SkhBlockBurn!]!
}
