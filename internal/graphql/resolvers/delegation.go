...
// OutstandingSSKH resolves the amount of outstanding sSKH tokens
// minted for this account.
func (del Delegation) OutstandingSSKH() (hexutil.Big, error) {
	val, err := repository.R().DelegationOutstandingSSKH(&del.Address, del.Delegation.ToStakerId)
	if err != nil {
		return hexutil.Big{}, err
	}
	return *val, nil
