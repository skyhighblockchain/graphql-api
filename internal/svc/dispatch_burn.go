...
// burnDispatcher implements dispatcher of new native SKH burns on the blockchain.
...
func (bud *burnDispatcher) process(tx *eventTrx, burn *types.SkhBurn) *types.SkhBurn {
	// no previous burn to check against? make a new record
	if burn == nil {
		return &types.SkhBurn{
	...
	if tx.blk.Number != burn.BlockNumber {
		...
		log.Debugf("collected block burn of %.4f SKH at #%d", val, burn.BlockNumber)

		if err := repo.StoreSkhBurn(burn); err != nil {
...
// burnedFee calculates the amount of burned SKHs from the transaction fee.
