...
import (
	...
	"skyhigh-api-graphql/internal/repository/db"
...
func connect(cfg *config.Config, log logger.Logger) (*cache.MemBridge, *db.MongoDbBridge, *rpc.SkhBridge, error) {
	...
	// create new Push RPC bridge
	rpcBridge, err := rpc.New(cfg, log)
	if err != nil {
		log.Criticalf("can not connect Push RPC interface, %s", err.Error())
		return nil, nil, nil, err
	}
