...
func (skh *SkhBridge) NativeTokenAddress() (*common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(skh.uniswapConfig.Router, skh.eth)
	if err != nil {
		skh.log.Errorf("Uniswap router contract not found; %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		skh.log.Errorf("Native token address not available; %s", err.Error())
		return nil, err
	}
...
func (skh *SkhBridge) UniswapPair(tokenA *common.Address, tokenB *common.Address) (*common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(skh.uniswapConfig.Core, skh.eth)
	if err != nil {
		skh.log.Errorf("Uniswap factory contract not found; %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		skh.log.Errorf("Uniswap pair not found for tokens %s and %s; %s", tokenA.String(), tokenB.String(), err.Error())
		return nil, err
	}
...
func (skh *SkhBridge) UniswapPairs(whiteListedOnly bool) ([]common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(skh.uniswapConfig.Core, skh.eth)
	if err != nil {
		skh.log.Errorf("Uniswap factory contract not found; %s", err.Error())
	...
	if err != nil || length == nil {
		skh.log.Error("Uniswap pairs array length not available")
		return nil, err
	}
	...
	for i := uint64(0); i < length.Uint64(); i++ {
		...
		if err != nil {
			skh.log.Errorf("error loading Uniswap pair; %s", err.Error())
			continue
		}

		// check the pair (is it on the white list?)
		// add the address to the list, if it's ok
		if !whiteListedOnly || skh.isUniswapPairWhitelisted(&adr) {
...
func (skh *SkhBridge) isUniswapPairWhitelisted(pair *common.Address) bool {
	// decode the pair address
	pairAddr := pair.String()

	// check the pair address against all the white listed pairs in config
	for _, addr := range skh.uniswapConfig.PairsWhiteList {
...
func (skh *SkhBridge) UniswapQuoteInput(
	amountA hexutil.Big,
	...
	if err != nil {
		skh.log.Errorf("can not calculate Uniswap quote; %s", err.Error())
		return hexutil.Big{}, err
	}
...
func (skh *SkhBridge) UniswapAmountsOut(amountIn hexutil.Big, tokens []common.Address) ([]hexutil.Big, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(skh.uniswapConfig.Router, skh.eth)
	if err != nil {
		skh.log.Errorf("Uniswap router contract not found; %s", err.Error())
	...
	return skh.uniswapLoadAmounts(contract.GetAmountsOut, (*big.Int)(&amountIn), tokens)
}
...
func (skh *SkhBridge) UniswapAmountsIn(amountOut hexutil.Big, tokens []common.Address) ([]hexutil.Big, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(skh.uniswapConfig.Router, skh.eth)
	if err != nil {
		skh.log.Errorf("Uniswap router contract not found; %s", err.Error())
	...
	return skh.uniswapLoadAmounts(contract.GetAmountsIn, (*big.Int)(&amountOut), tokens)
}

// uniswapLoadAmounts loads a list of swap amounts using the given loader function
// of the Uniswap router contract.
func (skh *SkhBridge) uniswapLoadAmounts(
	...
	if err != nil {
		skh.log.Errorf("can not load swap amounts; %s", err.Error())
		return nil, err
	}
...
func (skh *SkhBridge) UniswapTokens(pair *common.Address) ([]common.Address, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, skh.eth)
	if err != nil {
		skh.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		skh.log.Errorf("Uniswap pair %s token A not found; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		skh.log.Errorf("Uniswap pair %s token B not found; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (skh *SkhBridge) UniswapReserves(pair *common.Address) ([]hexutil.Big, error) {
	// get the reserves record from the contract
	rs, err := skh.uniswapReservesRecord(pair)
	if err != nil {
		skh.log.Errorf("Uniswap pair %s reserves not available; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (skh *SkhBridge) UniswapReservesTimeStamp(pair *common.Address) (hexutil.Uint64, error) {
	// get the reserves record from the contract
	rs, err := skh.uniswapReservesRecord(pair)
	if err != nil {
		skh.log.Errorf("Uniswap pair %s reserves not available; %s", pair.String(), err.Error())
...
func (skh *SkhBridge) UniswapCumulativePrices(pair *common.Address) ([]hexutil.Big, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, skh.eth)
	if err != nil {
		skh.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
	...
	if err != nil {
		skh.log.Errorf("Uniswap pair %s cumulative price A not available; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		skh.log.Errorf("Uniswap pair %s cumulative price B not available; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (skh *SkhBridge) uniswapReservesRecord(pair *common.Address) (*struct {
	Reserve0           *big.Int
	Reserve1           *big.Int
...
func (skh *SkhBridge) UniswapLastKValue(pair *common.Address) (hexutil.Big, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, skh.eth)
	if err != nil {
		skh.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
	...
	if err != nil {
		skh.log.Errorf("Uniswap pair %s coefficient K not available; %s", pair.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (skh *SkhBridge) UniswapPairContract(pairAddres *common.Address) (*contracts.UniswapPair, error) {
	contract, err := contracts.NewUniswapPair(*pairAddres, skh.eth)
	if err != nil {
		skh.log.Errorf("Uniswap pair contract %s not found; %s", pairAddres.String(), err.Error())
		return nil, err
	}
	return contract, nil
}
...
func (skh *SkhBridge) UniswapFactoryContract() (*contracts.UniswapFactory, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(skh.uniswapConfig.Core, skh.eth)
	if err != nil {
		skh.log.Errorf("Uniswap factory contract not found; %s", err.Error())
