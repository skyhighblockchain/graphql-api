...
func (skh *SkhBridge) GovernanceProposalsCount(gov *common.Address) (hexutil.Big, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, skh.eth)
	if err != nil {
		skh.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		skh.log.Errorf("can not count governance %s proposals; %s", gov.String(), err.Error())
		return hexutil.Big{}, err
	}
...
// specified by its id.
func (skh *SkhBridge) GovernanceProposal(gov *common.Address, id *hexutil.Big) (*types.GovernanceProposal, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, skh.eth)
	if err != nil {
		skh.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	// get the detail
	gp, err := skh.governanceProposalDetail(gc, gov, id.ToInt())
	if err != nil {
		skh.log.Errorf("proposal %d not available in %s; %s", id.ToInt().Uint64(), gov.String(), err.Error())
		return nil, err
	}
...
// GovernanceProposal provides a detail of Proposal of a governance contract
// specified by its id.
func (skh *SkhBridge) governanceProposalDetail(gc *contracts.Governance, govId *common.Address, id *big.Int) (*types.GovernanceProposal, error) {
	...
	ext, err := skh.GovernanceProposalDetails(&data.ProposalContract)
	if err != nil {
		return nil, err
	}
...
// specified by its id.
func (skh *SkhBridge) GovernanceProposalState(gov *common.Address, id *hexutil.Big) (*types.GovernanceProposalState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, skh.eth)
	if err != nil {
		skh.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		skh.log.Errorf("can not access governance %s proposal %d state; %s", gov.String(), id.ToInt().Int64(), err.Error())
		return nil, err
	}
...
// specified by its id.
func (skh *SkhBridge) GovernanceProposalDetails(prop *common.Address) (*govProposalExtended, error) {
	// get the proposal contract
	pp, err := contracts.NewGovernanceProposal(*prop, skh.eth)
	if err != nil {
		skh.log.Errorf("can not access governance proposal %s; %s", prop.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		skh.log.Errorf("governance proposal %s name not available; %s", prop.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		skh.log.Errorf("governance proposal %s description not available; %s", prop.String(), err.Error())
		return nil, err
	}
...
func (skh *SkhBridge) GovernanceOptionState(gov *common.Address, propId *hexutil.Big, optId *hexutil.Big) (*types.GovernanceOptionState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, skh.eth)
	if err != nil {
		skh.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	gs, err := skh.GovernanceOptionStateById(gc, propId, optId)
	if err != nil {
		skh.log.Errorf("governance %s proposal #%d state #%d not available; %s",
...
func (skh *SkhBridge) GovernanceOptionStates(gov *common.Address, propId *hexutil.Big, optRange int) ([]*types.GovernanceOptionState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, skh.eth)
	if err != nil {
		skh.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	for i := 0; i < optRange; i++ {
		...
		res[i], err = skh.GovernanceOptionStateById(gc, propId, (*hexutil.Big)(big.NewInt(int64(i))))
		if err != nil {
			skh.log.Errorf("unknown option #%d on governance %s; %s", i, gov.String(), err.Error())
			break
		}
	}
	return res, nil
...
func (skh *SkhBridge) GovernanceOptionStateById(gc *contracts.Governance, propId *hexutil.Big, optId *hexutil.Big) (*types.GovernanceOptionState, error) {
	// get the state
...
func (skh *SkhBridge) GovernanceVote(
	gov *common.Address,
	propId *hexutil.Big,
	from *common.Address,
	...
	if err != nil {
		skh.log.Errorf("can not access vote of %s on governance %s; %s", from.String(), gov.String(), err.Error())
		return nil, err
	}
...
func (skh *SkhBridge) GovernanceProposalsBy(gov *common.Address) ([]*types.GovernanceProposal, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, skh.eth)
	if err != nil {
		skh.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	// log what we do
	skh.log.Noticef("loading %d proposals of %s", maxProposalId.Uint64(), gov.String())
	...
	for i := maxProposalId.Int64(); i > 0; i-- {
		...
		gp, err := skh.governanceProposalDetail(gc, gov, big.NewInt(i))
		if err != nil {
			skh.log.Errorf("can not access governance %s proposal %d; %s", gov.String(), i, err.Error())
			return nil, err
		}

		// keep the proposal in the list
		skh.log.Noticef("found proposal #%d on %s", gp.Id.ToInt().Uint64(), gov.String())
		result = append(result, gp)
	}

	// log what we do
	skh.log.Debugf("%d proposals of %s loaded", maxProposalId.Uint64(), gov.String())
	return result, nil
}

// GovernanceProposalFee returns the fee payable for a new proposal
// in given Governance contract context.
func (skh *SkhBridge) GovernanceProposalFee(gov *common.Address) (hexutil.Big, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, skh.eth)
	if err != nil {
		skh.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		skh.log.Errorf("governance %s fee not available; %s", gov.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (skh *SkhBridge) GovernanceTotalWeight(ge *common.Address) (*hexutil.Big, error) {
	// get the contract
	goe, err := contracts.NewGovernable(*ge, skh.eth)
	if err != nil {
		skh.log.Errorf("can not access governable adapter %s; %s", ge.String(), err.Error())
		return nil, err
	}

	// get the total available voting weight
	w, err := goe.GetTotalWeight(skh.DefaultCallOpts())
	if err != nil {
		skh.log.Errorf("total weight not available on adapter %s; %s", ge.String(), err.Error())
		return nil, err
	}
