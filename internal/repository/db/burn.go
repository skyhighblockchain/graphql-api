...
// colBurns represents the name of the native SKH burns collection in database.
...
// StoreBurn stores the given native SKH burn record.
func (db *MongoDbBridge) StoreBurn(burn *types.SkhBurn) error {
	if burn == nil {
		return nil
	}
	...
	if sr.Err() != nil {
		...
		db.log.Errorf("could not load SKH burn at #%d; %s", burn.BlockNumber, sr.Err())
		return sr.Err()
	}

	// decode existing burn and update
	var ex types.SkhBurn
	if err := sr.Decode(&sr); err != nil {
		db.log.Errorf("could not decode SKH burn at #%d; %s", burn.BlockNumber, sr.Err())
		return sr.Err()
	}
...
// BurnList provides list of native SKH burns per blocks stored in the persistent database.
func (db *MongoDbBridge) BurnList(count int64) ([]types.SkhBurn, error) {
	col := db.client.Database(db.dbName).Collection(colBurns)
	...
	list := make([]types.SkhBurn, 0, count)

	for cr.Next(ctx) {
		var row types.SkhBurn
		if err := cr.Decode(&row); err != nil {
